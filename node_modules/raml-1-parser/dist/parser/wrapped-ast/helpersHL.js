...
var expander = require("../ast.core/expanderHL");
...
function extractParams(params, uri, ownerHl, propName, serializeMetadata) {
...
function dumpExpandableExample(ex, dumpXMLRepresentationOfExamples) {
    ...
    if (ex.hasScalarAnnotations()) {
        ...
        var saObj = {};
        Object.keys(sAnnotations).forEach(function (pName) {
            var aObj1 = toAnnotations(sAnnotations[pName]);
            if (Object.keys(aObj1).length > 0) {
                saObj[pName] = Object.keys(aObj1).map(function (x) { return aObj1[x]; });
            }
        });
...
function allTraits(hlNode, serializeMetadata) {
    ...
    return findTemplates(hlNode, function (d) { return universeHelpers.isTraitType(d); }, serializeMetadata);
}
...
function allResourceTypes(hlNode, serializeMetadata) {
    ...
    return findTemplates(hlNode, function (d) { return universeHelpers.isResourceTypeType(d); }, serializeMetadata);
}
exports.allResourceTypes = allResourceTypes;
function findTemplates(hlNode, filter, serializeMetadata) {
