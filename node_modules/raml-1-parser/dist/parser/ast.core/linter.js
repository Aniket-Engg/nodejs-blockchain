...
function createIssue(issueCode, message, node, isWarning, internalRange, forceScalar) {
    ...
    if (proxy.LowLevelProxyNode.isInstance(node.lowLevel())) {
        ...
        while (!proxyNode.primaryNode()) {
            if (!original) {
                var paramsChain = proxyNode.transformer() && proxyNode.transformer().paramNodesChain(proxyNode);
                if (paramsChain && paramsChain.length > 0) {
                    ...
                    for (var i_1 = 0; i_1 < paramsChain.length; i_1++) {
                        var pNode = paramsChain[i_1];
                        var ir = (i_1 == paramsChain.length - 1) ? internalRange : null;
                        var fs_1 = (i_1 == paramsChain.length - 1) ? forceScalar : false;
