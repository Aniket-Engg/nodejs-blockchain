...
exports.State = State;
var ReferencePatcherActionsAndConditionsFactory = (function () {
        ...
        if (transformer) {
            if (stringToPatch && stringToPatch.indexOf("<<") >= 0) {
                ...
                var newValue1 = transformer.transform(stringToPatch, true, function () { return doContinue; }, function (val, tr) {
                    ...
                    if (newVal == null) {
                        newVal = new referencePatcherHL.PatchedReference(null, val, collectionName, state.rootUnit, referencePatcherHL.PatchMode.DEFAULT);
                    }
                    var referencedUnit;
                    var newNS = newVal.namespace();
                    if (newNS) {
                        var referencedUsesInfo = byNSMap[newNS];
            ...
            if (key != null) {
                ...
                if (templatesCollection) {
                    if (newValue == null && stringToPatch.indexOf(".") < 0) {
                        newValue = new referencePatcherHL.PatchedReference("", stringToPatch, templatesCollection, state.rootUnit, referencePatcherHL.PatchMode.DEFAULT);
                    }
                    if (newValue) {
