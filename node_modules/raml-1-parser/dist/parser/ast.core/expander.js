...
var TraitsAndResourceTypesExpander = (function () {
    ...
    TraitsAndResourceTypesExpander.prototype.init = function (hlNode) {
    ...
    TraitsAndResourceTypesExpander.prototype.readGenerictData = function (r, obj, context, template, transformer, unitsChain) {
        ...
        if (typeof (value) == 'string') {
            ...
            var sv = value;
            var name_1 = sv.valueName();
            if (transformer) {
                name_1 = transformer.transform(name_1).value;
            }
            var scalarParamValues_1 = {};
            var scalarParams_1 = {};
            var structuredParams_1 = {};
            var _node = referencePatcher.getDeclaration(name_1, propName, unitsChain);
            if (_node) {
                var node = _node.wrapperNode();
                if (this.ramlVersion == 'RAML08') {
                    sv.children().forEach(function (x) { return scalarParamValues_1[x.valueName()] = x.lowLevel().value(); });
                }
                else {
                    sv.children().forEach(function (x) {
                        ...
                        if (llNode.resolvedValueKind() == yaml.Kind.SCALAR) {
                            scalarParamValues_1[x.valueName()] = llNode.value();
                            scalarParams_1[x.valueName()] = llNode;
                        }
                        else {
                            structuredParams_1[x.valueName()] = llNode;
                        }
                    });
                }
                var ds_1 = new DefaultTransformer(r, null, unitsChain);
                Object.keys(scalarParamValues_1).forEach(function (x) {
                    var q = ds_1.transform(scalarParamValues_1[x]);
                    //if (q.value){
                    if (q) {
                        if (typeof q !== "object") {
                            scalarParamValues_1[x] = q;
                        }
                    }
                    //}
                });
                var valTransformer = new ValueTransformer(template, name_1, unitsChain, scalarParamValues_1, scalarParams_1, structuredParams_1, transformer);
